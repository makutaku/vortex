...
from ..logging import LoggingConfig as LogConfig
from ..logging import PerformanceLogger, VortexLogger, configure_logging
from ..logging import get_logger as _get_logger
from ..logging import get_performance_logger as _get_performance_logger
from .config import ConfigManager, VortexConfig
...
def configure_logging_from_config(
    config: VortexConfig, service_name: str = "vortex", version: str = "unknown"
):
...
def configure_logging_from_manager(
...
def reconfigure_if_changed(config: VortexConfig):
    ...
    if (
        current_logging.level != new_logging.level
        or current_logging.format != new_logging.format
        or current_logging.output != new_logging.output
        or current_logging.file_path != new_logging.file_path
    ):
        configure_logging_from_config(config)
...
class HealthChecker:
    ...
    def run_checks(self) -> dict:
        ...
        results = {"status": "healthy", "timestamp": None, "checks": {}}
        ...
        for name, check_func in self.checks.items():
            try:
                ...
                self.logger.error(
                    f"Health check error: {name}: {e}", check_name=name, exc_info=True
                )
        ...
        self.logger.info(
            ...
            healthy_count=sum(
                1 for c in results["checks"].values() if c.get("healthy", False)
            ),
        )
...
def _check_config_system():
    ...
    try:
        ...
        return {"healthy": False, "details": f"Configuration system error: {e}"}
