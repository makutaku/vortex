...
class InputValidator:
    ...
    def validate_username(cls, username: str) -> ValidationResult:
        ...
        if len(username) < cls.MIN_USERNAME_LENGTH:
            return ValidationResult(
                False, f"Username must be at least {cls.MIN_USERNAME_LENGTH} characters"
            )

        if len(username) > cls.MAX_USERNAME_LENGTH:
            return ValidationResult(
                False, f"Username must not exceed {cls.MAX_USERNAME_LENGTH} characters"
            )
        ...
        if dangerous_chars:
            return ValidationResult(
                False, f"Username contains dangerous characters: {dangerous_chars}"
            )

        # Control character check
        if any(ord(char) < 32 or ord(char) == 127 for char in username):
            return ValidationResult(False, "Username contains control characters")
        ...
        unsafe_chars = [
            char for char in username if char not in cls.SAFE_USERNAME_CHARS
        ]
        if unsafe_chars:
            warnings.append(
                f"Username contains potentially unsafe characters: {unsafe_chars}"
            )
    ...
    def validate_password(cls, password: str) -> ValidationResult:
        ...
        if len(password) < cls.MIN_PASSWORD_LENGTH:
            return ValidationResult(
                False, f"Password must be at least {cls.MIN_PASSWORD_LENGTH} characters"
            )

        if len(password) > cls.MAX_PASSWORD_LENGTH:
            return ValidationResult(
                False, f"Password must not exceed {cls.MAX_PASSWORD_LENGTH} characters"
            )

        # Control character check (passwords can have more variety)
        if any(ord(char) < 32 and char not in ["\t"] for char in password):
            return ValidationResult(
                False, "Password contains prohibited control characters"
            )

        # Null byte check (critical security issue)
        if "\x00" in password:
            return ValidationResult(False, "Password contains null bytes")
    ...
    def validate_credentials(
        ...
        if not username_result.is_valid:
            errors.append(
                f"Username validation failed: {username_result.error_message}"
            )
        ...
        if not password_result.is_valid:
            errors.append(
                f"Password validation failed: {password_result.error_message}"
            )
    ...
    def validate_file_path(cls, file_path: str) -> ValidationResult:
        ...
        if len(file_path) > cls.MAX_INPUT_LENGTH:
            return ValidationResult(
                False, f"File path too long (max {cls.MAX_INPUT_LENGTH} characters)"
            )
        ...
        for pattern in dangerous_patterns:
            if pattern in file_path:
                return ValidationResult(
                    False, f"File path contains dangerous pattern: {pattern}"
                )
