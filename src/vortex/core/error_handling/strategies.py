...
class ErrorHandlingStrategy(Enum):
    ...
    RETURN_DEFAULT = (
        "return_default"  # Return default value (for operations with fallbacks)
    )
    LOG_AND_CONTINUE = (
        "log_continue"  # Log error and continue (for best-effort operations)
    )
    COLLECT_ERRORS = (
        "collect_errors"  # Collect errors and return them (for batch operations)
    )
...
class StandardizedErrorHandler:
    ...
    def handle_error(
        self, error: Exception, context: ErrorContext, strategy: ErrorHandlingStrategy
    ) -> Any:
    ...
    def _log_error(self, error: Exception, context: ErrorContext):
        ...
        correlation_part = (
            f"[{context.correlation_id}] " if context.correlation_id else ""
        )
        self.logger.error(
    ...
    def _fail_fast(self, error: Exception, context: ErrorContext):
        ...
        raise VortexError(
            f"{context.component}.{context.operation} failed: {error}"
        ) from error
    ...
    def decorator(func: Callable[..., T]) -> Callable[..., Union[T, None, Any]]:
        ...
        def wrapper(*args, **kwargs):
            ...
            context = ErrorContext(
                operation=operation, component=component, default_value=default_value
            )
...
def return_none_on_error(
    operation: str, component: str, error_types: Optional[tuple] = None
):
    """Decorator for optional operations that should return None on error."""
    return with_error_handling(
        ErrorHandlingStrategy.RETURN_NONE, operation, component, error_types=error_types
    )
...
def return_default_on_error(operation: str, component: str, default_value: Any):
    ...
    return with_error_handling(
        ErrorHandlingStrategy.RETURN_DEFAULT, operation, component, default_value
    )
...
def log_and_continue(operation: str, component: str):
    ...
    return with_error_handling(
        ErrorHandlingStrategy.LOG_AND_CONTINUE, operation, component
    )
