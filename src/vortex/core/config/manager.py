...
class ConfigManager:
    ...
    def _apply_general_env_overrides(
        self, config_data: Dict[str, Any], settings: VortexSettings
    ) -> None:
    ...
    def _apply_logging_env_overrides(
        self, config_data: Dict[str, Any], settings: VortexSettings
    ) -> None:
    ...
    def _apply_provider_env_overrides(
        self, config_data: Dict[str, Any], settings: VortexSettings
    ) -> None:
    ...
    def _apply_barchart_env_overrides(
        self, config_data: Dict[str, Any], settings: VortexSettings
    ) -> None:
    ...
    def save_config(self, config: Optional[VortexConfig] = None) -> None:
        ...
        if "general" in config_dict and "output_directory" in config_dict["general"]:
            config_dict["general"]["output_directory"] = str(
                config_dict["general"]["output_directory"]
            )
    ...
    def set_provider_config(
        ...
        from .models import BarchartConfig, IBKRConfig, YahooConfig
        ...
        try:
            ...
            raise ConfigurationValidationError(
                [f"Invalid {provider} configuration values: {e}"]
            )
        except Exception as e:
            raise ConfigurationValidationError(
                [f"Unexpected error configuring {provider}: {e}"]
            )
    ...
    def validate_provider_credentials(self, provider: str) -> bool:
        ...
        if provider == "barchart":
            return (
                config.providers.barchart.username is not None
                and config.providers.barchart.password is not None
            )
        elif provider == "yahoo":
    ...
    def _filter_none_values(self, data: Any) -> Any:
        ...
        if isinstance(data, dict):
            return {
                k: self._filter_none_values(v) for k, v in data.items() if v is not None
            }
        elif isinstance(data, list):
