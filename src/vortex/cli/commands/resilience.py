...
@click.option("--provider", "-p", help="Show stats for specific provider only")
...
def _show_summary(ux, stats: Dict[str, Dict[str, Any]]):
    ...
    ux.print_panel(
        (
            ...
            f"**Health Score:** {(healthy/total_breakers)*100:.1f}%"
            if total_breakers > 0
            else "No breakers active"
        ),
...
def _show_table(ux, stats: Dict[str, Dict[str, Any]]):
    ...
    for name, s in stats.items():
        ...
        failure_rate = (
            f"{s['failure_rate']*100:.1f}%" if s["failure_rate"] is not None else "N/A"
        )
        ...
        if last_failure:
            try:
                ...
                last_failure = (
                    last_failure[:19] if len(last_failure) > 19 else last_failure
                )
        else:
            last_failure = "Never"
