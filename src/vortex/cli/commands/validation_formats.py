        ...
        try:
            ...
            if validator:
                ...
                result["warnings"].append(
                    f"Unknown provider '{provider}' - skipping provider-specific validation"
                )
    ...
    def _load_dataframe(self, path: Path, result: dict):
        ...
        if path.suffix.lower() == ".csv":
            ...
            result["errors"].append(
                f"Unsupported file format for provider validation: {path.suffix}"
            )
            raise Exception(
                f"Unsupported file format for provider validation: {path.suffix}"
            )
    ...
    def _check_required_columns(
        ...
        for col_const, col_lower in zip(column_constants, required_columns):
            if col_lower not in df_columns:
                result["warnings"].append(
                    f"{provider_name} format missing '{col_const}' column"
                )

    def _check_date_columns(
        self, df_columns: list, date_variants: list, provider_name: str, result: dict
    ):
        """Check for date/time columns."""
        has_date = any(col in df_columns for col in date_variants)
        if not has_date:
            result["warnings"].append(
                f"{provider_name} format missing date/time column"
            )
...
class BarchartFormatValidator(BaseFormatValidator):
    ...
    def validate_format(self, df, result: dict):
        ...
        self._check_date_columns(
            df_columns_lower, ["date", "time", "datetime"], "Barchart", result
        )
    ...
    def _check_ohlcv_columns(self, df_columns: list, result: dict):
        ...
        self._check_required_columns(
            df_columns, required_columns, required_constants, "Barchart", result
        )
...
class YahooFormatValidator(BaseFormatValidator):
    ...
    def validate_format(self, df, result: dict):
        ...
        # Check date columns
        self._check_date_columns(
            df_columns_lower, ["date", "datetime"], "Yahoo", result
        )
    ...
    def _check_standard_ohlcv_columns(
        self, df_columns: list, result: dict, provider: str
    ):
        ...
        required_columns = [col.lower() for col in required_constants]
        self._check_required_columns(
            df_columns, required_columns, required_constants, provider, result
        )
    ...
    def _check_adj_close_column(self, df_columns: list, result: dict):
        ...
        if not has_adj_close:
            result["warnings"].append(
                f"Yahoo format typically includes '{ADJ_CLOSE_COLUMN}' column"
            )
...
class IbkrFormatValidator(BaseFormatValidator):
    ...
    def validate_format(self, df, result: dict):
        ...
        # Check date columns
        self._check_date_columns(df_columns_lower, ["date", "datetime"], "IBKR", result)
    ...
    def _check_ibkr_specific_columns(self, df_columns: list, result: dict):
        ...
        if WAP_COLUMN.lower() not in df_columns:
            result["warnings"].append(
                f"IBKR format typically includes '{WAP_COLUMN}' (Weighted Average Price) column"
            )
        if COUNT_COLUMN.lower() not in df_columns:
            result["warnings"].append(
                f"IBKR format typically includes '{COUNT_COLUMN}' (trade count) column"
            )
