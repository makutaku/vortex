...
from vortex.constants import DEFAULT_IBKR_PORT, ProviderConstants
...
from ..utils.config_utils import format_provider_status
...
@click.option(
    "--set-credentials", is_flag=True, help="Set credentials for specified provider"
)
@click.option(
    "--export", type=click.Path(path_type=Path), help="Export configuration to file"
)
@click.option(
    "--import",
    "import_file",
    ...
    if import_file:
        try:
            ...
            raise InvalidConfigurationError(
                "import_file", import_file, "valid TOML format"
            )
        return
...
def show_configuration(config_manager: ConfigManager) -> None:
    ...
    table.add_row(
        "Default Download Date Range",
        f"{default_start} to {default_end} (last 30 days)",
    )
    ...
    for provider in ["barchart", "yahoo", "ibkr"]:
        assets_file = Path(f"config/assets/{provider}.json")
        if assets_file.exists():
            default_assets.append(f"{provider}: {assets_file}")
    if not default_assets:
        assets_file = Path("config/assets/default.json")
        if assets_file.exists():
...
def show_provider_configuration(config_manager: ConfigManager, provider: str) -> None:
    ...
    if provider == "barchart":
        table.add_row(
            "Username", provider_config.get("username") or "[red]Not set[/red]"
        )
        table.add_row(
            "Password",
            "••••••••" if provider_config.get("password") else "[red]Not set[/red]",
        )
        table.add_row(
            "Daily Limit",
            str(
                provider_config.get(
                    "daily_limit",
                    ProviderConstants.Barchart.DEFAULT_DAILY_DOWNLOAD_LIMIT,
                )
            ),
        )
    elif provider == "yahoo":
...
def set_provider_credentials(config_manager: ConfigManager, provider: str) -> None:
    ...
    if provider == "barchart":
        ...
        daily_limit = Prompt.ask(
            ...
            default=str(ProviderConstants.Barchart.DEFAULT_DAILY_DOWNLOAD_LIMIT),
        )
        ...
        config_manager.set_provider_config(provider, {"enabled": True})
    ...
    if provider == "barchart":
        console.print(
            "[dim]  vortex download --provider barchart --symbol GC --start-date 2024-01-01[/dim]"
        )
    elif provider == "ibkr":
        console.print(
            "[dim]  Make sure TWS/Gateway is running and API is enabled[/dim]"
        )
