    ...
    try:
        ...
        for asset_class, instruments in assets_config.items():
            if not isinstance(instruments, dict):
                logging.warning(f"Skipping non-dict asset class '{asset_class}' in {assets_file_path}")
                continue
            
            for symbol, config in instruments.items():
                if not isinstance(config, dict):
                    logging.warning(f"Skipping non-dict config for symbol '{symbol}' in {assets_file_path}")
                    continue
        ...
        if not all_instruments:
            raise CLIError(f"No valid instruments found in assets file {assets_file_path}")
        
        logging.info(f"Loaded {len(all_instruments)} instruments from {assets_file_path}")
...
class SymbolResolver:
    ...
    def resolve_symbols(self, symbols: Optional[List[str]] = None, 
                       assets_file: Optional[Path] = None) -> Tuple[List[str], Dict[str, Any]]:
        ...
        handlers = [
            AssetsFileHandler(),
            DirectSymbolsHandler(),
            DefaultAssetsHandler()
        ]
...
class SymbolResolutionContext:
    ...
    def __init__(self, symbols: Optional[List[str]], assets_file: Optional[Path], provider: str):
...
class AssetsFileHandler(SymbolResolutionHandler):
    ...
    def handle(self, context: SymbolResolutionContext) -> Tuple[List[str], Dict[str, Any]]:
        ...
        logging.info(f"Using {len(symbols)} symbols from assets file: {context.assets_file}")
...
class DefaultAssetsHandler(SymbolResolutionHandler):
    ...
    def handle(self, context: SymbolResolutionContext) -> Tuple[List[str], Dict[str, Any]]:
        ...
        if default_file and default_file.exists():
            ...
            symbols = list(instrument_configs.keys())
            logging.info(f"Using {len(symbols)} symbols from default assets file: {default_file}")
            return symbols, instrument_configs
...
def resolve_symbols_and_configs(provider: str, symbols: Optional[List[str]] = None, 
                               assets_file: Optional[Path] = None) -> Tuple[List[str], Dict[str, Any]]:
    """Resolve symbols and their configurations from various sources.
