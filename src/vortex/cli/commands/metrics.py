...
def status(format: str):
    ...
    if not config.general.metrics.enabled:
        console.print(
            "[yellow]⚠️  Metrics collection is disabled in configuration[/yellow]"
        )
        return
    ...
    if format == "json":
        ...
        # Rich table format
        table = Table(
            title="Vortex Metrics Status", show_header=True, header_style="bold magenta"
        )
        ...
        table.add_row(
            "Endpoint",
            f"http://localhost:{config.general.metrics.port}{config.general.metrics.path}",
        )
...
def endpoint():
    ...
    console.print("🔗 Or integrate with monitoring tools like Grafana")


@metrics.command("test")
@click.option(
    "--samples", type=int, default=5, help="Number of test samples to generate"
)
def test(samples: int):
    ...
    if not config.general.metrics.enabled:
        console.print(
            "[yellow]⚠️  Metrics are disabled - enable in configuration first[/yellow]"
        )
        return
        ...
        for i in range(samples):
            ...
            try:
                ...
                if success:
                    ...
                    metrics_instance.record_download_failure(
                        provider, f"TEST{i}", "TestError"
                    )
            except Exception:
...
def summary(provider: Optional[str]):
    ...
    console.print(
        "   • Check monitoring stack with: docker compose -f docker/docker-compose.monitoring.yml up"
    )
