...
from .dependencies import get_dependency, is_available
...
def setup_logging(config_file: Optional[Path] = None, verbose: int = 0) -> None:
    ...
    if not is_available("config"):
        if verbose > 0:
            fallback_logger.debug(
                "Using fallback logging (config module not available)"
            )
        return
    ...
    try:
        ...
        configure_logging_from_manager = get_dependency(
            "config", "configure_logging_from_manager"
        )
        get_logger = get_dependency("config", "get_logger")

        if ConfigManager and configure_logging_from_manager and get_logger:
            config_manager = ConfigManager(config_file)
            configure_logging_from_manager(
                config_manager, service_name="vortex-cli", version=__version__
            )

            logger = get_logger("vortex.cli")
            logger.info(
                "Vortex CLI started", version=__version__, verbose_level=verbose
            )
        else:
            if verbose > 0:
                fallback_logger.debug(
                    "Config components not fully available, using fallback logging"
                )
        ...
        if config.get("execute"):
            ...
            if commands and commands.get("download"):
                ctx.invoke(
                    ...
                    **_convert_wizard_config_to_params(config),
                )
            else:
...
def register_commands():
    ...
    if resilience_commands and resilience_commands.get("resilience"):
        cli.add_command(resilience_commands["resilience"])
...
def main() -> None:
    ...
    get_logger_func = (
        get_dependency("config", "get_logger") if config_available else None
    )
