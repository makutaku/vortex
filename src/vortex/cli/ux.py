    ...
    def print(self):
        ...
        if self.ux.console:
            ...
            for col in self.columns:
                table.add_column(
                    col["name"], style=col["style"], justify=col["justify"]
                )
...
class CommandWizard:
    ...
    def run_config_wizard(self) -> Dict[str, Any]:
        ...
        if self.ux.confirm("Configure general settings?", False):
            ...
            config["backup_enabled"] = self.ux.confirm(
                "Enable automatic Parquet backups?", False
            )
...
def enhanced_error_handler(func: Callable) -> Callable:
    ...
    def wrapper(*args, **kwargs):
        try:
            ...
            ux.print_info(
                "💡 This might be a bug. Please report it at: https://github.com/makutaku/vortex/issues"
            )
            logger.error(
                f"Unexpected CLI error: {e}", command=func.__name__, exc_info=True
            )
            raise click.Abort()
    ...
    for cmd in available_commands:
        ...
        if command_lower in cmd.lower() or cmd.lower() in command_lower:
            ...
            if abs(len(command) - len(cmd)) <= 2:
                diff_count = sum(
                    1 for a, b in zip(command_lower, cmd.lower()) if a != b
                )
                if diff_count <= 2:
                    suggestions.append(cmd)
