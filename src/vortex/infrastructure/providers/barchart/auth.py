...
class BarchartAuth:
    ...
    def _create_session(self) -> requests.Session:
        ...
        session.headers.update({"User-Agent": NetworkConstants.SIMPLE_USER_AGENT})
        return session

    def login(self):
        """Authenticate with Barchart using credentials (bc-utils methodology)."""
        config = LoggingConfiguration(
            entry_msg="Logging in ...", success_msg="Logged in."
        )
        with LoggingContext(config):
            # First, get the login page to establish session (with timeout to prevent hanging)
            resp = self.session.get(
                self.BARCHART_LOGIN_URL, timeout=NetworkConstants.LOGIN_REQUEST_TIMEOUT
            )
            ...
            if not csrf_token:
                raise ValueError(
                    "CSRF token not found - authentication may have changed"
                )
            ...
            resp = self.session.post(
                self.BARCHART_LOGIN_URL,
                data=payload,
                allow_redirects=True,
                timeout=NetworkConstants.LOGIN_REQUEST_TIMEOUT,
            )
            ...
            if "login" in resp.url.lower():
                ...
                raise AuthenticationError(
                    "barchart", "Invalid Barchart credentials or login failed"
                )
            ...
            if resp.status_code != NetworkConstants.HTTP_OK:
                ...
                raise AuthenticationError(
                    "barchart", f"Login failed with status code: {resp.status_code}"
                )

            # Verify we have necessary cookies for API access (optional check for testing)
            if (
                hasattr(self.session, "cookies")
                and "laravel_session" not in self.session.cookies
            ):
                ...
                logger = logging.getLogger(__name__)
                logger.warning(
                    "Login successful but missing expected laravel_session cookie"
                )

    def logout(self):
        """Logout from Barchart session."""
        config = LoggingConfiguration(
            entry_msg="Logging out ...", success_msg="Logged out."
        )
        with LoggingContext(config):
            self.session.get(
                self.BARCHART_LOGOUT_URL, timeout=NetworkConstants.SHORT_REQUEST_TIMEOUT
            )
    ...
    def make_api_request(
        ...
        try:
            ...
            logger.error(
                f"API request timeout: {url} (timeout: {NetworkConstants.DEFAULT_REQUEST_TIMEOUT})"
            )
            raise
