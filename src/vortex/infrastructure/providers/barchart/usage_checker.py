...
class BarchartUsageChecker:
    ...
    def check_server_usage(self) -> Optional[int]:
        ...
        try:
            ...
            if home_response.status_code != NetworkConstants.HTTP_OK:
                self.logger.debug(
                    f"Cannot access home page for usage check: {home_response.status_code}"
                )
                return None
            ...
            payload = {"onlyCheckPermissions": "true"}  # This is the bc-utils secret!
            ...
            if resp.status_code == NetworkConstants.HTTP_OK:
                try:
                    ...
                    if json_data.get("error") is not None:
                        self.logger.debug(
                            f"Usage check error: {json_data.get('error')}"
                        )
                        return None
                    ...
                    if json_data.get("success"):
                        ...
                        self.logger.debug(
                            f"bc-utils usage success: {json_data['success']}, count: {used_count}"
                        )
                        return used_count
                    else:
    ...
    def validate_daily_limit(self) -> None:
        ...
        if current_usage is not None:
            ...
            self.logger.info(
                f"Usage check: {current_usage}/{self.daily_limit} downloads used, {remaining} remaining"
            )
        else:
            self.logger.warning(
                "Could not verify usage count - proceeding with download"
            )
