...
class BarchartClient:
    ...
    def request_download(
        ...
        try:
            ...
            payload = self._build_download_request_payload(
                history_csrf_token, symbol, frequency_attributes, start_date, end_date
            )
            resp = self.session.post(
                self.BARCHART_DOWNLOAD_URL,
                headers=headers,
                data=payload,
            ...
            if self._metrics:
                self._metrics.record_provider_request(
                    "barchart", "download", duration, True
                )
                if hasattr(resp, "content") and resp.content:
                    # Estimate row count from response size (rough approximation)
                    estimated_rows = (
                        len(resp.content) // 50
                    )  # ~50 bytes per row estimate
                    self._metrics.record_download(
                        "barchart", symbol, estimated_rows, True
                    )

            logging.debug(
                f"POST {self.BARCHART_DOWNLOAD_URL}, "
            ...
            if self._metrics:
                self._metrics.record_provider_request(
                    "barchart", "download", duration, False
                )
    ...
    def fetch_usage(self, url: str, xsrf_token: str) -> tuple[dict, str]:
        ...
        try:
            with LoggingContext(config):
                ...
                if self._metrics:
                    self._metrics.record_provider_request(
                        "barchart", "usage_check", duration, True
                    )
            ...
            if self._metrics:
                self._metrics.record_provider_request(
                    "barchart", "usage_check", duration, False
                )
                self._metrics.record_error(type(e).__name__, "barchart", "usage_check")
            raise

    def _build_download_request_payload(
        self,
        history_csrf_token: str,
        symbol: str,
        frequency_attributes: FrequencyAttributes,
        ...
        return {
            ...
            "dividends": getattr(
                self, "dividends", ProviderConstants.Barchart.DEFAULT_DIVIDENDS
            ),
            "backadjust": getattr(
                self, "backadjust", ProviderConstants.Barchart.DEFAULT_BACKADJUST
            ),
            "dbar": getattr(self, "dbar", ProviderConstants.Barchart.DEFAULT_DBAR),
            "custombar": getattr(
                self, "custombar", ProviderConstants.Barchart.DEFAULT_CUSTOMBAR
            ),
            "volume": getattr(
                self, "volume", ProviderConstants.Barchart.DEFAULT_VOLUME
            ),
            "openInterest": getattr(
                self, "openInterest", ProviderConstants.Barchart.DEFAULT_OPEN_INTEREST
            ),
            "splits": getattr(
                self, "splits", ProviderConstants.Barchart.DEFAULT_SPLITS
            ),
        }
    ...
    def _build_usage_payload(self) -> dict:
        ...
        return {
            "type": getattr(
                self, "usage_type", ProviderConstants.Barchart.DEFAULT_USAGE_TYPE
            )
        }
