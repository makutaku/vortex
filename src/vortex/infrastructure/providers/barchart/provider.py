...
from ..interfaces import BarchartHTTPClient, HTTPClientProtocol
...
class BarchartDataProvider(DataProvider):
    ...
    def __init__(
        ...
        self.usage_checker = BarchartUsageChecker(
            self.auth, self.client, config.daily_limit
        )
    ...
    def _extract_csrf_token(self, home_response) -> Optional[str]:
        ...
        try:
            ...
            return self._handle_provider_error(
                e, "extract_csrf_token", strategy=ErrorHandlingStrategy.RETURN_NONE
            )
    ...
    def login(self):
        ...
        try:
            ...
            return self._handle_provider_error(
                e, "login", strategy=ErrorHandlingStrategy.FAIL_FAST
            )
    ...
    def _get_frequency_attributes(self) -> list[FrequencyAttributes]:
        ...
        return [
            FrequencyAttributes(
                Period("1d"),
                get_min_start_date(Period("1d")),
                get_max_range(Period("1d")),
                {
                    "max_bars": self.config.max_bars_per_download,
                    "frequency_name": "daily",
                },
            ),
            FrequencyAttributes(
                Period("1h"),
                get_min_start_date(Period("1h")),
            ...
            FrequencyAttributes(
                Period("30m"),
                get_min_start_date(Period("30m")),
                get_max_range(Period("30m")),
                {
                    "max_bars": self.config.max_bars_per_download,
                    "frequency_name": "30minute",
                },
            ),
            FrequencyAttributes(
                Period("15m"),
                get_min_start_date(Period("15m")),
                get_max_range(Period("15m")),
                {
                    "max_bars": self.config.max_bars_per_download,
                    "frequency_name": "15minute",
                },
            ),
            FrequencyAttributes(
                Period("5m"),
                get_min_start_date(Period("5m")),
                get_max_range(Period("5m")),
                {
                    "max_bars": self.config.max_bars_per_download,
                    "frequency_name": "5minute",
                },
            ),
            FrequencyAttributes(
                Period("1m"),
                get_min_start_date(Period("1m")),
    ...
    def _fetch_historical_data(
        ...
        if isinstance(instrument, Future):
            symbol = (
                ...
                if hasattr(instrument, "get_symbol")
                else str(instrument)
            )
            url = self.get_historical_quote_url(instrument)
            return self._fetch_historical_data_(
                symbol,
                frequency_attributes,
                start_date,
                end_date,
                url,
                FUTURES_SOURCE_TIME_ZONE,
                instrument,
            )
            ...
            symbol = (
                f"^{base_symbol}" if not base_symbol.startswith("^") else base_symbol
            )
            url = self.get_historical_quote_url(instrument)
            return self._fetch_historical_data_(
                symbol,
    ...
    def _fetch_historical_data_(
        ...
        try:
            ...
            if current_usage is not None:
                logger.info(
                    f"bc-utils server usage before download: {current_usage} "
                    f"downloads used today (configured limit: {self.get_daily_limit()})"
                )
            else:
                logger.info(
                    f"bc-utils usage check unavailable (configured limit: {self.get_daily_limit()})"
                )
            ...
            df = self._fetch_via_bc_utils_download(
                instrument,
                frequency_attributes,
                start_date,
                end_date,
    ...
    def _fetch_via_bc_utils_download(
        self,
        instrument: str,
        frequency_attributes: FrequencyAttributes,
        start_date,
        ...
        try:
            return self._perform_bc_utils_download(
                instrument,
                frequency_attributes,
                start_date,
                end_date,
    ...
    def _perform_bc_utils_download(
        self,
        instrument: str,
        frequency_attributes: FrequencyAttributes,
        start_date,
        ...
        home_response = self.auth.session.get(
            self.config.base_url, timeout=self.config.request_timeout
        )
        ...
        if frequency_attributes.frequency == Period("1d"):
            ...
            payload[
                "fields"
            ] = "tradeTime.format(m/d/Y),openPrice,highPrice,lowPrice,lastPrice,priceChange,percentChange,volume"
            payload["type"] = "eod"
            ...
            payload[
                "fields"
            ] = "tradeTime.format(Y-m-d H:i),openPrice,highPrice,lowPrice,lastPrice,volume"
        ...
        logger.info(
            f"Using /my/download endpoint with meta CSRF token for {instrument}"
        )
        ...
        if response.status_code != 200:
            from vortex.exceptions.providers import (
                VortexConnectionError as ConnectionError,
            )

            raise ConnectionError(
                "barchart",
                f"Download failed with status: {response.status_code}. Response: {response.text[:300]}...",
            )
        ...
        if response.text and has_csv_data:
            ...
            if post_download_usage is not None:
                logger.info(
                    f"bc-utils server usage after download: {post_download_usage} "
                    f"downloads used today (configured limit: {self.get_daily_limit()})"
                )
            ...
            if self._raw_storage:
                try:
                    ...
                    logger.warning(
                        f"Failed to save Barchart raw data trail: {raw_error}"
                    )

            return self._process_bc_utils_csv_response(
                response.text, frequency_attributes.frequency, tz
            )
        else:
    ...
    def _process_bc_utils_csv_response(
        self, csv_data: str, frequency, tz: str
    ) -> Optional[DataFrame]:
