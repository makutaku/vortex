...
from vortex.infrastructure.resilience.recovery import (
    DataProviderRecoveryPolicy,
    ErrorRecoveryManager,
)
from vortex.infrastructure.resilience.retry import RetryStrategy, provider_retry
...
class ResilientDataProvider(DataProvider):
    ...
    def __init__(self, provider_name: str, fallback_providers: Optional[list] = None):
        ...
        self.circuit_breaker = get_circuit_breaker(
            f"provider_{provider_name}", cb_config
        )

        # Initialize error recovery manager
        recovery_policy = DataProviderRecoveryPolicy(
            max_retry_attempts=3, fallback_providers=self.fallback_providers
        )
        self.recovery_manager = ErrorRecoveryManager(recovery_policy)

        logger.info(
            "ResilientDataProvider initialized",
    ...
    def login(self):
        ...
        try:
            ...
            CorrelationIdManager.add_context_metadata(
                provider=self.provider_name, operation_type="authentication"
            )
    ...
    def _perform_login(self):
        ...
        if rand < 0.1:  # 10% chance of connection error
            ...
            raise AuthenticationError(
                self.provider_name, "Invalid credentials provided", http_code=401
            )
        elif rand < 0.2:  # 5% chance of rate limit
            raise RateLimitError(self.provider_name, wait_time=120, daily_limit=1000)
        ...
        try:
            ...
            try:
                ...
                if recovery_result.success:
                    logger.info(
                        "Data fetch recovered successfully",
                        provider=self.provider_name,
                        correlation_id=correlation_id,
    ...
    def _perform_data_fetch(
        ...
        if rand < 0.05:  # 5% chance of connection error
            ...
            raise RateLimitError(self.provider_name, wait_time=60, daily_limit=500)
...
def create_resilient_yahoo_provider() -> ResilientDataProvider:
    ...
    return ResilientDataProvider(
        provider_name="yahoo", fallback_providers=["barchart", "alpha_vantage"]
    )
...
def create_resilient_barchart_provider() -> ResilientDataProvider:
    ...
    return ResilientDataProvider(
        provider_name="barchart", fallback_providers=["yahoo", "ibkr"]
    )
