...
from ..config import CircuitBreakerConfig, YahooProviderConfig
...
class YahooDataProvider(DataProvider):
    ...
    def validate_configuration(self) -> bool:
        ...
        try:
            ...
            self.logger.warning(f"Yahoo provider validation failed: {e}")
            return False
    ...
    def _get_frequency_attributes(self) -> List[FrequencyAttributes]:
        return [
            FrequencyAttributes(
                Period.Monthly, min_start=None, properties={"interval": "1mo"}
            ),
            FrequencyAttributes(
                Period.Weekly, min_start=None, properties={"interval": "1wk"}
            ),
            FrequencyAttributes(
                Period.Daily, min_start=None, properties={"interval": "1d"}
            ),
            FrequencyAttributes(
                Period.Hourly,
            ...
            FrequencyAttributes(
                Period.Minute_30,
                min_start=timedelta(
                    days=ProviderConstants.Yahoo.INTRADAY_30MIN_DAYS_LIMIT
                ),
            ...
            FrequencyAttributes(
                Period.Minute_15,
                min_start=timedelta(
                    days=ProviderConstants.Yahoo.INTRADAY_15MIN_DAYS_LIMIT
                ),
            ...
            FrequencyAttributes(
                Period.Minute_5,
                min_start=timedelta(
                    days=ProviderConstants.Yahoo.INTRADAY_5MIN_DAYS_LIMIT
                ),
            ...
            FrequencyAttributes(
                Period.Minute_1,
                min_start=timedelta(
                    days=ProviderConstants.Yahoo.INTRADAY_1MIN_DAYS_LIMIT
                ),
    ...
    def _fetch_data_using_injected_fetcher(
        self,
        symbol: str,
        interval: str,
        start_date: datetime,
        ...
        try:
            ...
            df = self._data_fetcher.fetch_historical_data(
            ...
            if df.empty:
                logger.debug(
                    f"Yahoo Finance returned empty data for {symbol} ({interval}) "
            ...
            logger.debug(
                f"Successfully fetched {len(df)} rows for {symbol} ({interval})"
            )
            return df
            ...
            logger.error(
                f"Yahoo Finance API error for {symbol} ({interval}): {type(e).__name__}: {e}"
            )
            raise ConnectionError(
                "yahoo", f"Failed to fetch data for {symbol}: {e}"
            ) from e
