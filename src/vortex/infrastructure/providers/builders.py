...
class BarchartProviderBuilder(ProviderBuilder):
    ...
    def build(self) -> BarchartDataProvider:
        ...
        auth = self._auth_handler or BarchartAuth(
...
class YahooProviderBuilder(ProviderBuilder):
    ...
    def with_cache_settings(
        self, cache_enabled: bool = True, cache_ttl_hours: int = 24
    ):
    ...
    def with_validation_settings(
        self, validate_data_types: bool = True, repair_data: bool = True
    ):
    ...
    def build(self) -> YahooDataProvider:
        ...
        # Create dependencies if not injected
        cache_manager = self._cache_manager or (
            YahooCacheManager() if config.cache_enabled else None
        )
...
class IBKRProviderBuilder(ProviderBuilder):
    ...
    def with_connection_settings(
        self, host: str = "localhost", port: int = 7497, client_id: Optional[int] = None
    ):
    ...
    def with_timeout_settings(
        self, connection_timeout: int = 30, data_timeout: int = 120
    ):
        """Configure timeout settings."""
        if not self._config:
            self._config = IBKRProviderConfig()
...
def create_barchart_provider_with_defaults(
...
def create_ibkr_provider_with_connection(
    host: str = "localhost", port: int = 7497
) -> IbkrDataProvider:
