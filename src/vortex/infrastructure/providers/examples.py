...
from .builders import barchart_builder, ibkr_builder, yahoo_builder
from .config import BarchartProviderConfig, IBKRProviderConfig, YahooProviderConfig
from .factory import ProviderFactory
from .metrics import get_all_metrics, get_metrics_collector
...
def example_basic_provider_creation():
    ...
    barchart = factory.create_provider(
        "barchart", {"username": "your_username", "password": "your_password"}
    )

    yahoo = factory.create_provider(
        "yahoo", {"cache_enabled": True, "validate_data_types": True}
    )

    ibkr = factory.create_provider("ibkr", {"host": "localhost", "port": 7497})
...
def example_factory_with_builders():
    ...
    ibkr = factory.create_provider_with_builder(
        "ibkr",
        lambda b: b.with_connection_settings("localhost", 7497).with_timeout_settings(
            45, 120
        ),
    )
...
def example_dependency_injection():
    ...
    yahoo = yahoo_builder().with_cache_manager(CustomYahooCache()).build()
...
def example_observability_and_metrics():
    ...
    # Create providers
    barchart = barchart_builder().with_credentials("user", "pass").build()
...
def example_complete_workflow():
    ...
    for name, provider in providers.items():
        try:
            ...
            with provider._metrics_collector.track_operation(
                "test_operation", symbol="TEST"
            ):
                import time
    ...
    for provider_name, metrics in all_metrics.items():
        logger.info(
            f"{provider_name}: {metrics.total_operations} ops, "
