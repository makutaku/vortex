...
class ProviderMetrics:
    ...
    performance_buckets: Dict[str, int] = field(default_factory=lambda: {
        '<100ms': 0,
        '100-500ms': 0,
        '500ms-1s': 0,
        '1-5s': 0,
    ...
    def update(self, operation: OperationMetrics):
        ...
        if operation.success:
            ...
            if operation.error_type:
                self.error_counts[operation.error_type] = self.error_counts.get(operation.error_type, 0) + 1
    ...
    def hourly_success_rate(self) -> float:
        ...
        if self.hourly_operations == 0:
            return 100.0
        return ((self.hourly_operations - self.hourly_failures) / self.hourly_operations) * 100
...
class ProviderMetricsCollector:
    ...
    @contextmanager
    def track_operation(self, operation_name: str, correlation_id: Optional[str] = None, **additional_data):
    ...
    def get_health_score(self) -> float:
        ...
        with self._lock:
            ...
            base_score = metrics.hourly_success_rate if metrics.hourly_operations > 0 else metrics.success_rate
            ...
            critical_errors = ['ConnectionError', 'AuthenticationError', 'DataProviderError']
            for error_type in critical_errors:
                if error_type in metrics.error_counts and metrics.error_counts[error_type] > 3:
                    base_score *= 0.8
