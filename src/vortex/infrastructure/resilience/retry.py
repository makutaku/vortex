...
class BackoffStrategy(ABC):
    ...
    @abstractmethod
    def calculate_delay(
        self, attempt: int, base_delay: float, max_delay: float, **kwargs
    ) -> float:
...
class ExponentialBackoffStrategy(BackoffStrategy):
    ...
    def __init__(
        self, multiplier: float = 2.0, jitter: bool = False, jitter_max: float = 0.1
    ):
...
class RetryManager:
    ...
    def __init__(self, policy: Optional[RetryPolicy] = None):
        ...
        self.strategy_map = {
            ...
            RetryStrategy.EXPONENTIAL_BACKOFF: ExponentialBackoffStrategy(
                multiplier=self.policy.multiplier, jitter=False
            ),
        ...
        for attempt in range(1, self.policy.max_attempts + 1):
            try:
                ...
                if not self._should_retry(e, attempt):
                    logger.info(
                        f"Not retrying {func.__name__} due to non-retryable exception",
    ...
    def _calculate_delay(
        ...
        if (
            ...
            and self.policy.rate_limit_respect_retry_after
        ):
...
class ProviderRetryManager(RetryManager):
    ...
    def __init__(self, provider_name: str, custom_policy: Optional[RetryPolicy] = None):
        ...
        policy = (
            custom_policy
            or provider_policies.get(provider_name.lower())
            or RetryPolicy()
        )
        super().__init__(policy)

        logger.info(
            f"ProviderRetryManager initialized for {provider_name}",
            max_attempts=self.policy.max_attempts,
            strategy=self.policy.strategy.value,
        )
