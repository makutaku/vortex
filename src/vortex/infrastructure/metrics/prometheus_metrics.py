...
class VortexMetrics:
    ...
    def __init__(self):
        ...
        self.active_correlations = Gauge(
            "vortex_active_correlations", "Number of active correlation contexts"
        )

        self.system_info = Info("vortex_system_info", "Vortex system information")
        ...
        self.config_loads_total = Counter(
            "vortex_config_loads_total", "Total configuration loads", ["status"]
        )
    ...
    def start_metrics_server(self, port: int = 8000, threaded: bool = True):
        ...
        if threaded:
            self._server_thread = threading.Thread(
                target=self._start_server_thread, args=(port,), daemon=True
            )
    ...
    def _set_system_info(self):
        ...
        try:
            ...
            self.system_info.info(
                {"version": "unknown", "python_version": sys.version.split()[0]}
            )
    ...
    def record_provider_request(
        ...
        self.provider_requests_total.labels(
            provider=provider.lower(), operation=operation, status=status
        ).inc()

        self.provider_request_duration.labels(
            provider=provider.lower(), operation=operation
        ).observe(duration)

    def record_download(
        self, provider: str, symbol: str, row_count: int, success: bool
    ):
        ...
        self.downloads_total.labels(
            provider=provider.lower(), status=status, symbol=symbol
        ).inc()

        if success and row_count > 0:
            self.download_rows.labels(provider=provider.lower(), symbol=symbol).observe(
                row_count
            )
    ...
    def record_storage_operation(
        self, operation: str, storage_type: str, duration: float, success: bool
    ):
        ...
        self.storage_operation_duration.labels(
            operation=operation, storage_type=storage_type
        ).observe(duration)
...
def initialize_metrics(
    port: int = 8000, enabled: bool = True
) -> Optional[VortexMetrics]:
