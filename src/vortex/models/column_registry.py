...
from .column_constants import REQUIRED_DATA_COLUMNS
...
class ColumnMappingRegistry:
    ...
    def get_mapping(self, provider_name: str) -> ProviderColumnMapping:
        ...
        if provider_lower not in self._mappings:
            raise ValueError(
                f"No column mapping registered for provider: {provider_name}"
            )
        return self._mappings[provider_lower]

    def get_column_mapping(
        self, provider_name: str, df_columns: List[str]
    ) -> Dict[str, str]:
    ...
    def get_provider_expected_columns(
