...
class InvalidConfigurationError(ConfigurationError):
    ...
    def __init__(self, field: str, value: Any, expected: str):
        ...
        help_text = (
            f"Please check the configuration for '{field}' and ensure "
            f"it matches the expected format: {expected}"
        )
        user_action = (
            "Run 'vortex config --provider <provider> --help' for valid values"
        )
        context = ExceptionContext(
            help_text=help_text, error_code="CONFIG_INVALID", user_action=user_action
        )
...
class MissingConfigurationError(ConfigurationError):
    ...
    def __init__(self, field: str, config_location: Optional[str] = None):
        ...
        help_text = "Set this value using 'vortex config --provider <provider> --set-credentials'"
        if config_location:
            help_text += f" or add it to {config_location}"
        context = ExceptionContext(help_text=help_text, error_code="CONFIG_MISSING")
        super().__init__(message, context)
...
class ConfigurationValidationError(ConfigurationError):
    ...
    def __init__(self, errors: List[str]):
        ...
        context = ExceptionContext(
            help_text=help_text, error_code="CONFIG_VALIDATION", user_action=user_action
        )
        super().__init__(message, context)
