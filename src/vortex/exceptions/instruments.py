...
class InvalidInstrumentError(InstrumentError):
    ...
    def __init__(self, symbol: str, reason: str):
        ...
        help_text = (
            "Check the symbol format and ensure it's supported by the selected provider"
        )
...
class UnsupportedInstrumentError(InstrumentError):
    ...
    def __init__(
        self, symbol: str, provider: str, supported_types: Optional[List[str]] = None
    ):
        from .base import ExceptionContext
        ...
        context = ExceptionContext(
            help_text=help_text, error_code="UNSUPPORTED_INSTRUMENT"
        )
        super().__init__(message, context)
