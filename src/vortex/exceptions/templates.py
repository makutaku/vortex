...
class ErrorMessageTemplates:
    ...
    PROVIDER_RATE_LIMITED = "Provider {provider}: Rate limit exceeded ({current}/{limit})"
    PROVIDER_DATA_NOT_FOUND = "Provider {provider}: No data found for {symbol} ({period}) from {start_date} to {end_date}"
    ...
    UNSUPPORTED_INSTRUMENT = "Unsupported instrument type '{instrument_type}' for provider {provider}"
