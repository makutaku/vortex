...
Tests the BarchartClient class including download requests, allowance checking,
...
class TestBarchartClientInit:
    ...
    def test_url_constants(self):
        ...
        assert BarchartClient.BARCHART_ALLOWANCE_URL == 'https://www.barchart.com/my/download'
...
class TestBarchartClientDownloadRequest:
    ...
    def test_request_download_success(self, client, sample_freq_attrs):
        ...
        hist_csrf_token = "test-hist-csrf"
        symbol = "AAPL"
        ...
        with patch('logging.debug') as mock_debug:
            result = client.request_download(
                xsrf_token, hist_csrf_token, symbol, sample_freq_attrs, 
    ...
    def test_request_download_headers_construction(self, client, sample_freq_attrs):
    ...
    def test_request_download_payload_construction(self, client, sample_freq_attrs):
        ...
        assert payload['_token'] == hist_csrf_token
...
@pytest.mark.unit  
class TestBarchartClientAllowance:
    """Test allowance checking functionality."""
    
    def test_fetch_allowance_success(self, client):
        """Test successful allowance fetch."""
        # Mock allowance response
        allowance_data = {"success": True, "count": 50, "limit": 150}
        mock_response = Mock()
        ...
        with patch('logging.debug') as mock_debug:
            result_allowance, result_token = client.fetch_allowance(url, xsf_token)
    ...
    def test_fetch_allowance_headers(self, client):
        """Test allowance request headers construction."""
        mock_response = Mock()
        ...
        client.fetch_allowance(url, xsf_token)
        
        call_args = client.session.post.call_args
        headers = call_args[1]['headers']
        
        assert headers['Content-Type'] == 'application/x-www-form-urlencoded; charset=UTF-8'
        assert headers['X-CSRF-TOKEN'] == xsf_token
        assert headers['X-Requested-With'] == 'XMLHttpRequest'
        assert headers['Referer'] == url
    
    def test_fetch_allowance_payload(self, client):
        """Test allowance request payload."""
        mock_response = Mock()
        mock_response.text = '{"success": true}'
        client.session.post.return_value = mock_response
        client.auth.get_xsrf_token.return_value = "token"
...
class TestBarchartClientStaticMethods:
    ...
    def test_build_download_request_payload(self, sample_freq_attrs):
        """Test download request payload construction."""
        hist_csrf_token = "test-csrf"
        symbol = "AAPL" 
        start_date = datetime(2024, 3, 15)
        end_date = datetime(2024, 3, 20)
        
        payload = BarchartClient._build_download_request_payload(
            hist_csrf_token, symbol, sample_freq_attrs, start_date, end_date
        )
    ...
    def test_build_allowance_request_headers(self):
        """Test allowance request headers construction."""
        url = "https://www.barchart.com/allowance"
        xsf_token = "allowance-token"
        
        headers = BarchartClient._build_allowance_request_headers(url, xsf_token)
        
        expected = {
            'Content-Type': 'application/x-www-form-urlencoded; charset=UTF-8',
            'X-CSRF-TOKEN': 'allowance-token',
            'X-Requested-With': 'XMLHttpRequest',
            'Referer': 'https://www.barchart.com/allowance'
        }
        
        assert headers == expected
    
    def test_build_allowance_payload(self):
        """Test allowance request payload construction."""
        payload = BarchartClient._build_allowance_payload()
...
class TestBarchartClientIntegration:
    ...
    def test_full_download_request_flow(self, client, sample_freq_attrs):
        ...
        # Execute request
        result = client.request_download(
            "xsrf-token", "csrf-token", "AAPL", sample_freq_attrs,
    ...
    def test_allowance_and_token_refresh_flow(self, client):
        """Test allowance check with token refresh."""
        # Mock allowance response
        allowance_data = {"success": True, "count": 25, "limit": 150}
        mock_response = Mock()
        mock_response.text = json.dumps(allowance_data)
        client.session.post.return_value = mock_response
        ...
        # Verify results
        assert allowance == allowance_data
        assert token == new_token
        assert allowance["count"] == 25
        assert allowance["success"] is True
