...
class TestProvidersCommand:
    ...
    def test_list_providers_basic(self, mock_config_manager_class, mock_get_registry, runner, mock_registry):
        ...
        mock_config_manager.load_config.return_value = {'providers': {}}
        mock_config_manager_class.return_value = mock_config_manager
        
        # Mock check_provider_configuration
        with patch('vortex.cli.commands.providers.check_provider_configuration') as mock_check_config:
            mock_check_config.return_value = {'configured': False}
    ...
    def test_list_providers_with_status_colors(self, mock_get_available, mock_get_registry, runner, mock_registry):
        """Test provider listing shows status colors."""
        mock_get_registry.return_value = mock_registry
        mock_get_available.return_value = [
            {'name': 'barchart', 'status': 'available', 'description': 'Barchart provider'},
            {'name': 'yahoo', 'status': 'missing_deps', 'description': 'Yahoo provider', 'missing': ['dependency1']},
            {'name': 'ibkr', 'status': 'error', 'description': 'IBKR provider', 'error': 'Import error'}
        ]
    ...
    @patch('vortex.cli.commands.providers.get_provider_registry')
    @patch('vortex.cli.commands.providers.get_available_providers')
    def test_list_providers_empty(self, mock_get_available, mock_get_registry, runner):
        ...
        assert result.exit_code == 0
        assert 'No providers found' in result.output or result.output.strip() == ""

    @patch('vortex.cli.commands.providers.get_provider_registry')
    @patch('vortex.cli.commands.providers.check_provider_configuration')
    def test_test_single_provider_success(self, mock_check_config, mock_get_registry, runner, mock_registry):
        ...
        assert result.exit_code == 0
        assert 'barchart' in result.output
        assert 'success' in result.output.lower() or 'successful' in result.output.lower()
        mock_check_config.assert_called_once_with('barchart')

    @patch('vortex.cli.commands.providers.get_provider_registry')
    @patch('vortex.cli.commands.providers.check_provider_configuration')
    def test_test_single_provider_failure(self, mock_check_config, mock_get_registry, runner, mock_registry):
    ...
    @patch('vortex.cli.commands.providers.get_provider_registry')
    @patch('vortex.cli.commands.providers.check_provider_configuration')
    def test_test_all_providers(self, mock_check_config, mock_get_registry, runner, mock_registry):
        """Test testing all providers."""
        mock_get_registry.return_value = mock_registry
        mock_registry.list_plugins.return_value = ['barchart', 'yahoo']
        mock_check_config.side_effect = [
        ...
        assert 'barchart' in result.output
        assert 'yahoo' in result.output
        assert mock_check_config.call_count == 2
        mock_check_config.assert_has_calls([call('barchart'), call('yahoo')])

    @patch('vortex.cli.commands.providers.get_provider_registry')
    def test_test_nonexistent_provider(self, mock_get_registry, runner, mock_registry):
    ...
    @patch('vortex.cli.commands.providers.get_provider_registry')
    def test_info_provider_basic(self, mock_get_registry, runner, mock_registry):
    ...
    @patch('vortex.cli.commands.providers.get_provider_registry')
    def test_info_provider_with_config(self, mock_get_registry, runner, mock_registry):
    ...
    @patch('vortex.cli.commands.providers.get_provider_registry')
    def test_info_nonexistent_provider(self, mock_get_registry, runner, mock_registry):
        """Test getting info for nonexistent provider."""
        mock_get_registry.return_value = mock_registry
        mock_registry.get_plugin.side_effect = KeyError('Provider not found')
    ...
    @patch('vortex.cli.commands.providers.get_provider_registry')
    @patch('vortex.cli.commands.providers.get_available_providers')
    def test_no_options_shows_help_or_list(self, mock_get_available, mock_get_registry, runner, mock_registry):
    ...
    def test_list_providers_console_output(self, mock_console, mock_get_registry, runner, mock_registry):
        """Test that provider listing uses console for rich output."""
        mock_get_registry.return_value = mock_registry
        
        with patch('vortex.cli.commands.providers.get_available_providers') as mock_get_available:
            mock_get_available.return_value = [
                {'name': 'barchart', 'status': 'available', 'description': 'Barchart provider'}
            ]
    ...
    @patch('vortex.cli.commands.providers.get_provider_registry')
    @patch('vortex.cli.commands.providers.check_provider_configuration')
    def test_test_provider_with_progress(self, mock_check_config, mock_get_registry, runner, mock_registry):
        """Test that provider testing shows progress indicator."""
        mock_get_registry.return_value = mock_registry
        mock_check_config.return_value = {'status': 'success', 'message': 'OK'}
        ...
        mock_check_config.side_effect = slow_check
    ...
    @patch('vortex.cli.commands.providers.get_provider_registry')
    @patch('vortex.cli.commands.providers.check_provider_configuration')
    def test_test_multiple_providers_summary(self, mock_check_config, mock_get_registry, runner, mock_registry):
        ...
        assert 'yahoo' in result.output  
        assert 'ibkr' in result.output
        assert mock_check_config.call_count == 3

    @patch('vortex.cli.commands.providers.get_provider_registry')
    def test_info_provider_detailed(self, mock_get_registry, runner, mock_registry):
    ...
    @patch('vortex.cli.commands.providers.get_provider_registry')
    @patch('vortex.cli.commands.providers.ConfigManager')
    def test_info_provider_with_config_status(self, mock_config_manager_class, mock_get_registry, runner, mock_registry):
    ...
    @patch('vortex.cli.commands.providers.get_provider_registry')
    @patch('vortex.cli.commands.providers.get_available_providers')
    def test_list_providers_table_format(self, mock_get_available, mock_get_registry, runner, mock_registry):
        """Test that provider list is formatted as a table."""
        mock_get_registry.return_value = mock_registry
        mock_get_available.return_value = [
            {'name': 'barchart', 'status': 'available', 'description': 'Barchart provider'},
            {'name': 'yahoo', 'status': 'available', 'description': 'Yahoo Finance provider'},
            {'name': 'ibkr', 'status': 'missing_deps', 'description': 'Interactive Brokers provider'}
        ]
        
        with patch('vortex.cli.commands.providers.Table') as mock_table:
    ...
    @patch('vortex.cli.commands.providers.get_provider_registry')
    @patch('vortex.cli.commands.providers.check_provider_configuration')
    def test_test_provider_exception_handling(self, mock_check_config, mock_get_registry, runner, mock_registry):
        """Test provider testing handles exceptions gracefully."""
        mock_get_registry.return_value = mock_registry
        mock_check_config.side_effect = Exception("Network error")
        
        result = runner.invoke(providers, ['--test', 'barchart'])
        
        # Should handle exception gracefully and not crash
        assert 'error' in result.output.lower() or 'exception' in result.output.lower() or result.exit_code != 0

    @patch('vortex.cli.commands.providers.get_provider_registry')
    def test_info_provider_exception_handling(self, mock_get_registry, runner, mock_registry):
        """Test provider info handles exceptions gracefully."""
        mock_get_registry.return_value = mock_registry
        mock_registry.get_plugin.side_effect = Exception("Plugin error")
    ...
    def test_command_logging(self, mock_logger, runner):
        """Test that command operations are properly logged."""
        with patch('vortex.cli.commands.providers.get_provider_registry'):
            with patch('vortex.cli.commands.providers.get_available_providers', return_value=[]):
                result = runner.invoke(providers, ['--list'])
